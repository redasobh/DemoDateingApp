    ...
    public class DatingRepository : IDatingRepository
    {
        ...
        public async Task<PagedList<Message>> GetMessagesForUser(MessageParams messageParams)
        {
            ...
            switch (messageParams.MessageContainer)
            {
                ...
                break;                
                default:
                messages=messages.Where(u=>u.RecipientId==messageParams.UserId &&u.RecipientDelete ==false && u.IsRead==false);
                break;
        ...
        public async Task<IEnumerable<Message>> GetMessagesThread(int userId, int recipientId)
        {
            ...
            .Include(u=>u.Recipient).ThenInclude(p => p.Photos).Where(m => m.RecipientId == userId &&m.RecipientDelete== false
            && m.SenderId == recipientId || m.RecipientId == recipientId && m.SenderId == userId && m.SenderDelete == false).OrderByDescending(m => m.MessageSent).ToListAsync();
            return messages;
        }
    }
}